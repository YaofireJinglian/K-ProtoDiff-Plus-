...
class CustomDataset(Dataset):
    ...
    def read_data(filepath, name=''):
        ...
        # Paper datasets from the Autoformer bundle contain a timestamp column,
        # while Energy and TimeGAN Stocks are already numeric.  Selecting
        # numeric columns keeps one loader valid for all nine datasets and
        # prevents timestamps from silently producing object arrays.
        df = df.select_dtypes(include=[np.number])
        if df.shape[1] == 0:
            raise ValueError(f'no numeric time-series columns found in {filepath}')
        data = df.values
        scaler = MinMaxScaler()
