...
class EEGDataset(Dataset):
    def __init__(
        ...
        if self.save2npy:
            np.save(
                os.path.join(self.dir, f'eeg_ground_truth_{self.window}_train.npy'),
                self.rawdata,
            )
            normalized = (
                unnormalize_to_zero_to_one(self.samples)
                if self.auto_norm else self.samples
            )
            np.save(
                os.path.join(self.dir, f'eeg_norm_truth_{self.window}_train.npy'),
                normalized,
            )
